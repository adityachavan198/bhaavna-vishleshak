...
def samplehandling(sample, lexicons, classification):
    ...
    with open(sample, 'r', encoding="utf8") as f:
        contents = f.read()
        pbar = tqdm(total=len(contents.split('\n')))
        for line in contents.split('\n'):
            data = line.strip('\n')
            if data:
                all_words = word_tokenize(data)
...
def create_feature_set(pos, neg):
    ...
    print("="*45," IN create_feature_set")
    featuresets += samplehandling(pos, lexicons, 1)
    print("="*45," IN create_feature_set 1st completed")
    featuresets += samplehandling(neg, lexicons, 0)
    print("="*45," IN create_feature_set 2nd completed")
...
def create_test_data_for_unigram(pos, neg, size):
    ...
    testset = []
    print("="*45,"IN create_test_data_for_unigram")
    for file in [pos, neg]:
    #for file in [neg,]:
        print(file)
        with open(file, "r", encoding="utf8") as f:
            content = f.read()
            pbar = tqdm(total=len(content.split('$')[:size]))
            for line in content.split('$')[:size]:
                line = line.strip('\n')
                if not line:
                    pbar.update(1)
                    continue
                print("Before Translator",line)
                line = translator.translate(line, dest="english").text
                print("After Translator",line)
    ...
    print("="*45,"OUT create_test_data_for_unigram completed")
    return testset


def create_classfier_for_tf():
    featureset = create_feature_set('pos_english.txt', 'neg_english.txt')
    print("="*45," IN create_classfier_for_tf","---------","create_feature_set completed \n processing...")
    ...
    with open("decisiontree.pkl", "wb") as f:
        print("dumping tree.pkl .....")
        pickle.dump(clf, f)
        print("dumped tree.pkl")
